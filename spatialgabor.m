...
function [Eim, Oim, Aim, Pim] = spatialgabor(im, central_frequency, angle, sigmax, sigmay, showfilter)
    ...
    evenFilter = exp(-(x.^2/sigmax^2 + y.^2/sigmay^2)/2)...
	     .*cos(2*pi*central_frequency*x);
    
    oddFilter = exp(-(x.^2/sigmax^2 + y.^2/sigmay^2)/2)...
	     .*sin(2*pi*central_frequency*x);    
